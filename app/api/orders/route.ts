  ...
  try {
    ...
    if (statusFilter) {
      ...
      if (validStatuses.includes(statusFilter)) {
        ...
        console.log(`[${requestId}] Filtering by status: ${statusFilter}`);
      } else {
        console.log(`[${requestId}] Invalid status filter: ${statusFilter}`);
        return NextResponse.json(
    ...
    const adminSession = await getAuthPayload(request, "admin_token");
    if (adminSession?.role === "admin") {
      console.log(`[${requestId}] Admin access granted - fetching orders with filters`);
      ...
      console.log(`[${requestId}] Found ${allRecords.length} orders (total: ${totalCount})`);
      
      // Return flat array for admin dashboard compatibility
      return NextResponse.json(allRecords, { 
        status: 200,
        headers: {
    ...
    if (statusFilter && statusFilter !== "All") {
      customerFilter.status = statusFilter;
    }
    ...
    console.log(`[${requestId}] Found ${records.length} orders for user ${userId} (total: ${totalCount})`);
    ...
    if (records.length > 0) {
      ...
      if (statusFilter && statusFilter !== "All") {
        emailFilter.status = statusFilter;
      }
      const emailOrders = await Order.find(emailFilter);
      if (emailOrders.length > 0) {
        console.log(`[${requestId}] ⚠️ Found ${emailOrders.length} orders with matching email but user: null`);
      }
    }

    // Return flat array for customer dashboard compatibility
    return NextResponse.json(records, { 
      status: 200,
      headers: {
        'X-Total-Count': totalCount.toString(),
