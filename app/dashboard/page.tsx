...
export default function CustomerDashboardPage() {
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-900 flex flex-col font-sans overflow-x-hidden">
      <Navbar />

      <section className="flex-grow w-full max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10 animate-fade-in-up">
        {/* ===== HEADER SECTION ===== */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-gray-200">
          <div className="min-w-0">
            <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 truncate">
              Welcome back, <span className="text-orange-500">{user.fullName.split(" ")[0]}</span> 👋
            </h1>
            <p className="text-gray-500 text-sm mt-1">
              Manage your account, track orders, and view purchase history.
            </p>
            {user.role === "admin" && (
              <span className="inline-flex items-center gap-1.5 text-xs font-bold text-orange-600 bg-orange-100 px-3 py-1 rounded-full mt-2 border border-orange-200">
                <Shield size={14} /> Admin Access - Viewing All Orders
              </span>
            )}
          </div>
          <div className="flex gap-2 flex-shrink-0">
            <button
              onClick={handleRefresh}
              disabled={refreshing}
              className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-700 hover:text-gray-900 hover:bg-gray-50 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium whitespace-nowrap shadow-sm hover:shadow"
            >
              <RefreshCw size={16} className={refreshing ? "animate-spin" : ""} />
              {refreshing ? "Refreshing..." : "Refresh"}
            </button>
            <button
              onClick={handleLogout}
              className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-red-50 border border-red-200 text-red-600 hover:bg-red-600 hover:text-white transition-all duration-200 text-sm font-medium whitespace-nowrap shadow-sm hover:shadow"
            >
              <LogOut size={16} /> Sign Out
            </button>
          </div>
        ...
        <div className="space-y-3 pt-4">
          {securityAlert && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-yellow-700 text-sm font-medium">{securityAlert}</p>
                <button
                  onClick={() => setSecurityAlert(null)}
                  className="text-yellow-600 text-xs font-medium underline mt-1 hover:text-yellow-800 transition-colors"
                >
                  Dismiss
                </button>
              </div>
          ...
          {error && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <button
                  onClick={handleRefresh}
                  className="text-red-600 text-xs font-medium underline mt-1 hover:text-red-800 transition-colors"
                >
                  Try again
                </button>
              </div>
        ...
        {/* ===== INFO CARDS ===== */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-6">
          <div className="bg-white border border-gray-200 rounded-2xl p-5 flex items-center gap-4 hover:border-orange-300 transition-all duration-300 hover:shadow-lg shadow-sm">
            <div className="p-3 bg-blue-50 rounded-xl text-blue-600 shrink-0">
              <User size={20} />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Name</p>
              <p className="font-bold truncate text-gray-900 text-sm">{user.fullName}</p>
            </div>
          </div>

          <div className="bg-white border border-gray-200 rounded-2xl p-5 flex items-center gap-4 hover:border-purple-300 transition-all duration-300 hover:shadow-lg shadow-sm">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="font-bold truncate text-gray-900 text-sm font-mono">{user.email}</p>
            </div>
          </div>

          <div className="bg-white border border-gray-200 rounded-2xl p-5 flex items-center gap-4 hover:border-green-300 transition-all duration-300 hover:shadow-lg shadow-sm">
            <div className="p-3 bg-green-50 rounded-xl text-green-600 shrink-0">
              <Eye size={20} />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Verified Orders</p>
              <p className="font-black text-2xl text-gray-900">{verifiedOrdersCount}</p>
            </div>
          </div>
        </div>
        ...
        {orders.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-6">
            {counts.pending > 0 && (
              <div className="bg-white border border-orange-200 rounded-xl p-4 text-center hover:border-orange-300 transition-all duration-300 hover:shadow-md shadow-sm">
                <p className="text-2xl font-black text-orange-600">{counts.pending}</p>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-0.5">Pending</p>
              </div>
            )}
            {counts.processed > 0 && (
              <div className="bg-white border border-blue-200 rounded-xl p-4 text-center hover:border-blue-300 transition-all duration-300 hover:shadow-md shadow-sm">
                <p className="text-2xl font-black text-blue-600">{counts.processed}</p>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-0.5">Processed</p>
              </div>
            )}
            {counts.dispatched > 0 && (
              <div className="bg-white border border-purple-200 rounded-xl p-4 text-center hover:border-purple-300 transition-all duration-300 hover:shadow-md shadow-sm">
                <p className="text-2xl font-black text-purple-600">{counts.dispatched}</p>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-0.5">Dispatched</p>
              </div>
            )}
            {counts.delivered > 0 && (
              <div className="bg-white border border-green-200 rounded-xl p-4 text-center hover:border-green-300 transition-all duration-300 hover:shadow-md shadow-sm">
                <p className="text-2xl font-black text-green-600">{counts.delivered}</p>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-0.5">Delivered</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="pt-8">
          <div className="flex flex-wrap items-center justify-between gap-3 pb-4">
            ...
            {orders.length > 0 && (
              <div className="flex items-center gap-3">
                <span className="text-[10px] text-green-700 flex items-center gap-1.5 bg-green-100 px-2.5 py-1 rounded-full">
                  <Shield size={12} /> Verified
                </span>
                {lastUpdated && (
                  <span className="text-[10px] text-gray-500">
          ...
          {ordersLoading ? (
            ...
            <div className="space-y-4">
              {orders.map((order, index) => {
                ...
                return (
                  <div
                    key={order._id}
                    className="bg-white border border-gray-200 rounded-2xl p-5 hover:border-orange-300 transition-all duration-300 hover:shadow-lg shadow-sm animate-fade-in-up"
                  ...
                  >
                    <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0 space-y-3">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="font-mono text-[10px] sm:text-xs font-bold text-orange-600 bg-orange-50 px-2.5 py-1 rounded-full border border-orange-200 flex items-center gap-1.5">
                            <Hash size={12} />
                            #{order._id.substring(order._id.length - 6).toUpperCase()}
                          </span>
                          ...
                          {user.role === "admin" && order.user && (
                            <span className="text-[8px] text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">
                              User: {order.user.toString().slice(-6)}
                            </span>
                          )}
                        </div>

                        <p className="text-sm text-gray-700 line-clamp-2">{order.itemsSummary}</p>

                        <div className="flex flex-wrap items-center gap-4 text-xs">
                          <span className="text-gray-500 flex items-center gap-1.5">
                            <Phone size={13} />
                            <span className="text-gray-800 font-mono">{order.phone}</span>
                          </span>
                          <span className="text-gray-500 flex items-center gap-1.5">
                            <CreditCard size={13} />
                            <span
                          ...
                          <button
                            onClick={() => toggleOrderExpansion(order._id)}
                            className="text-gray-500 hover:text-gray-900 transition-colors flex items-center gap-1.5 text-xs font-medium"
                          >
                            {isExpanded ? (
                              <>
                                <ChevronUp size={14} /> Less
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="space-y-1.5 text-xs">
                            ...
                            <p className="flex justify-between">
                              ...
                              <span className="text-gray-900 font-mono">{order.customerEmail}</span>
                            </p>
                            <p className="flex justify-between">
                              <span className="text-gray-500">Phone:</span>
                              <span className="text-gray-900 font-mono">{order.phone}</span>
                            </p>
                          </div>
                        </div>
                        <div className="space-y-2">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-12 text-center shadow-sm animate-fade-in">
        ...
        <div className="pt-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <a
              href="/shop"
              className="group relative bg-white border border-gray-200 rounded-2xl p-5 hover:border-orange-400 transition-all duration-300 hover:shadow-lg shadow-sm overflow-hidden min-h-[72px] flex items-center"
            >
              {/* Background glow effect */}
              <div className="absolute inset-0 bg-gradient-to-r from-orange-500/0 via-orange-500/0 to-orange-500/0 group-hover:from-orange-500/10 group-hover:via-orange-500/0 group-hover:to-orange-500/10 transition-all duration-500" />
              
              <div className="flex items-center justify-between w-full relative z-10">
                <span className="font-bold flex items-center gap-3 text-sm sm:text-base text-gray-800 group-hover:text-gray-900">
                  <Settings size={18} className="text-orange-500 shrink-0" /> 
                  <span className="truncate">Browse Parts</span>
                </span>
                <ArrowRight
                  size={18}
                  className="text-gray-400 group-hover:text-gray-800 transition-colors shrink-0 ml-4"
                />
              </div>
            </a>
            ...
            <a
              href="/shop"
              className="group relative bg-white border border-gray-200 rounded-2xl p-5 hover:border-green-400 transition-all duration-300 hover:shadow-lg shadow-sm overflow-hidden min-h-[72px] flex items-center"
            >
              <div className="absolute inset-0 bg-gradient-to-r from-green-500/0 via-green-500/0 to-green-500/0 group-hover:from-green-500/10 group-hover:via-green-500/0 group-hover:to-green-500/10 transition-all duration-500" />
            ...
            <a
              href="/about"
              className="group relative bg-white border border-gray-200 rounded-2xl p-5 hover:border-blue-400 transition-all duration-300 hover:shadow-lg shadow-sm overflow-hidden min-h-[72px] flex items-center"
            >
              <div className="absolute inset-0 bg-gradient-to-r from-blue-500/0 via-blue-500/0 to-blue-500/0 group-hover:from-blue-500/10 group-hover:via-blue-500/0 group-hover:to-blue-500/10 transition-all duration-500" />
