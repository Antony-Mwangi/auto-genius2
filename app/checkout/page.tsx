...
//   const [email, setEmail] = useState(""); // ADDED: State tracking for customer or guest emails
...
//             setEmail(profileData.user.email || ""); // ADDED: Autofill email address context if verified
...
//         body: JSON.stringify({ name, email, phone, paymentMethod, cart, total }), // UPDATED: Payload parameters map directly to your backend matrix
//       });
...
//           {/* ADDED: Email Address Input block for identity matching and invoices */}
//           <div>
//             <label className="block text-xs uppercase font-black text-gray-400 mb-1 tracking-wider">Email Address</label>
//             <input 
//               type="email" 
//               required 
...
export default function CheckoutPage() {
  ...
  // Calculate subtotal (product costs only)
  ...
  return (
    <main className="min-h-screen bg-[#0b0f14] text-white p-4 sm:p-6 flex items-center justify-center relative antialiased font-sans">
      ...
      <form onSubmit={handlePlaceOrder} className="w-full max-w-md bg-[#111827] border border-white/10 rounded-2xl p-6 sm:p-8 space-y-5 shadow-2xl">
        ...
        <div className="space-y-2">
          <label className="block text-xs uppercase font-black text-gray-400 tracking-wider">Delivery Location</label>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {Object.entries(SHIPPING_LOCATIONS).map(([key, { label, cost }]) => (
              <button
                ...
                className={`p-3 rounded-xl border font-bold text-xs transition flex flex-col items-center justify-center gap-0.5 cursor-pointer text-center ${
                  shippingLocation === key
                    ? cost === 0
                      ? "border-purple-500 bg-purple-500/10 text-purple-400 shadow-md shadow-purple-500/5"
                      : "border-orange-500 bg-orange-500/10 text-orange-400 shadow-md shadow-orange-500/5"
                    : "border-white/10 bg-[#0b0f14] text-gray-400 hover:border-white/20"
                }`}
              >
                ...
                {shippingLocation === key && (
                  <span className="text-[8px] mt-0.5 font-black uppercase tracking-wider">
                    ✓ Selected
                  </span>
                )}
              </button>
        ...
        <div className="bg-[#0b0f14] p-4 rounded-xl border border-white/5 space-y-2">
          ...
          {shippingCost > 0 && (
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-400">Shipping ({SHIPPING_LOCATIONS[shippingLocation].label}):</span>
              <span className="text-orange-400 font-semibold">+ Ksh {shippingCost.toLocaleString()}</span>
            </div>
          )}
          ...
          {shippingCost === 0 && shippingLocation === "Pickup at Shop" && (
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="text-purple-400 font-semibold">Free (Pickup)</span>
            </div>
          )}
          
          <div className="border-t border-white/5 pt-2 flex justify-between items-center">
            <span className="text-sm font-bold text-gray-300">Total Settlement:</span>
            <span className="text-white font-black text-lg tracking-wide">Ksh {total.toLocaleString()}</span>
          </div>
        </div>
