...
//   const markAsDispatched = async (orderId: string, mongoId: string) => {
//     try {
//       const res = await fetch("/api/orders", {
//         method: "PUT",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ id: mongoId, status: "Dispatched" }),
//       });

//       if (res.ok) {
//         setOrders(prev => prev.map(o => o.id === orderId ? { ...o, status: "Dispatched" } : o));
//       } else {
//         alert("Failed to update status changes on database nodes.");
//       }
//     } catch (err) {
...
//                   {pendingOrdersCount > 0 && <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse inline-block" />}
//                 </span>
//                 {pendingOrdersCount > 0 && <span className="bg-red-500 text-white text-xs px-2.5 py-0.5 rounded-full font-bold">{pendingOrdersCount}</span>}
//               </button>
//             </nav>
//           </div>
...
//                   className="w-full bg-[#0b0f14] border border-white/10 rounded-xl p-3 outline-none text-sm font-bold text-[#0b0f14] placeholder-gray-500 focus:bg-white focus:border-orange-500 transition" 
//                 />
//               </div>
...
//                 <p className="text-gray-400 text-xs sm:text-sm mt-1">Real-time incoming client checkout store metrics.</p>
//               </div>
//               {pendingOrdersCount > 0 && <span className="bg-red-500 text-white font-extrabold text-[10px] sm:text-xs px-2.5 py-1 rounded-full animate-pulse shadow-lg whitespace-nowrap">{pendingOrdersCount} NEW</span>}
//             </div>
            
//             <div className="space-y-4">
//               {orders.length > 0 ? (
//                 orders.map(order => (
//                   <div key={order.id} className={`border rounded-2xl p-4 sm:p-5 shadow-xl transition flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative overflow-hidden ${order.status === "Pending" ? "bg-[#1f1d1a] border-orange-500/30" : "bg-[#111827] border-white/10"}`}>
...
//                     <div className="shrink-0 w-full md:w-auto text-right border-t border-white/5 pt-3 md:border-0 md:pt-0">
//                       {order.status === "Pending" ? (
//                         <button 
//                           onClick={() => markAsDispatched(order.id, order.mongoId)} 
//                           className="w-full md:w-auto bg-orange-500 hover:bg-orange-400 text-white text-xs font-extrabold px-4 py-2.5 rounded-xl transition cursor-pointer shadow-md flex items-center justify-center gap-1.5"
//                         >
...
//                         </button>
//                       ) : (
//                         <span className="inline-flex w-full md:w-auto items-center justify-center gap-1.5 text-xs font-bold text-green-400 bg-green-500/10 border border-green-500/20 px-3 py-2 rounded-xl">
//                           <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
            ...
            <form onSubmit={handleProductSubmit} className="bg-[#111827] border border-orange-500/10 rounded-2xl p-5 sm:p-8 space-y-5 shadow-xl relative">
              ...
              <div>
                ...
                <input 
                  ...
                  value={product.name} 
                  onChange={e => setProduct(prev => ({ ...prev, name: e.target.value }))} 
                  className="w-full bg-[#0b0f14] border border-white/10 rounded-xl p-3 outline-none text-sm font-bold text-white placeholder-gray-500 focus:bg-white focus:border-orange-500 transition" 
                />
              </div>
